...
Zenith 3 Flight Software as flown in CanSat Competition 2025 on 7 Jun (commenting changed post flight)
Freshmen student team from the University of Alabama in Huntsville's Space Hardware Club

Authors:
    - Akhil Samaraju (SimMode, testing, structure, state-machine, LED visual interface / Party Mode)
    - Caleb Wiley (SimMode, logic, debugging, testing, magnetometer callibration, and loop optimization)
    - Barrett Twinning (sensor integration, GPS usage, and library integration)
    - Diego Romero-Cardona (ground-station integration and state-machine)

**Relics from previous versions have not been removed**
**Custom hardware designed by Barrett Twinning**
*/
...
// Debugging
const bool DEBUG = false; // leave off for flights and testing
void debugCheckpoint(const char *message)
{
    static int checkpoint = 0;
    if (DEBUG)
    {
        checkpoint++; // keeps track of placement and order of debug statements
...
#define GND_CAM_CTRL 10 // PID board pin
#define BLADE_CAM_CTRL 11
#define LED_DATA 5
#define NUM_LEDS 5
#define TEAM_ID "3195"
...
bool telemetryEnabled = false; // SHOULD ALWAYS BE FALSE SO TELEMETRY DOESN'T START AUTOMATICALLY AND ONLY STARTS WHEN CX_ON COMMAND IS RECIEVED (per mission guide)
...
// all-important party mode
void partyModeEffect()
{
    if (!partyMode)
        return;
    if (millis() - lastPartyUpdateTime < PARTY_UPDATE_INTERVAL)
        return;
...
// callibrateable altitude calculation
...
// compute accelerometer calibrations
...
// history for use in logic and averaging
...
// command handler and groundstation integration
void handleCommand(const char *command)
{
    ...
    int cmdType = 0;
    if (strcmp(field1, "CX") == 0)
        cmdType = 1;
    else if (strcmp(field1, "ST") == 0)
        cmdType = 2;
    else if (strcmp(field1, "SIM") == 0)
        cmdType = 3;
    else if (strcmp(field1, "SIMP") == 0)
        cmdType = 4;
    else if (strcmp(field1, "CAL") == 0)
        cmdType = 5;
    else if (strcmp(field1, "MEC") == 0)
        cmdType = 6;
    else if (strcmp(field1, "PARTY") == 0)
        cmdType = 7;
...
void updateFlightState(float altitude, float velocity, float x, float y, float z)
{
    ...
    case PROBE_RELEASE:
        ...
        if ((abs(velocity) < 0.5))
        { // Relaxed velocity threshold and use probeReleaseTime
            landedVelocities++;
        }
...
// initialize, settup sensors, and calibrate
void setup()
{
...
void loop()
{
    ...
    {
        if (rpmCount > 0 && timeDifference > 10)
        { // Only calculate if valid data
    ...
    if (isnan(pressure) || isinf(pressure))
        pressure = 0.0;
    if (isnan(currentVoltage) || isinf(currentVoltage))
        currentVoltage = 0.0;
    if (isnan(gyroX) || isinf(gyroX))
        gyroX = 0.0;
    if (isnan(gyroY) || isinf(gyroY))
        gyroY = 0.0;
    if (isnan(gyroZ) || isinf(gyroZ))
        gyroZ = 0.0;
    if (isnan(accelX) || isinf(accelX))
        accelX = 0.0;
    if (isnan(accelY) || isinf(accelY))
        accelY = 0.0;
    if (isnan(accelZ) || isinf(accelZ))
        accelZ = 0.0;
    if (isnan(magEvent1.magnetic.x) || isinf(magEvent1.magnetic.x))
        magEvent1.magnetic.x = 0.0;
    if (isnan(magEvent1.magnetic.y) || isinf(magEvent1.magnetic.y))
        magEvent1.magnetic.y = 0.0;
    if (isnan(magEvent1.magnetic.z) || isinf(magEvent1.magnetic.z))
        magEvent1.magnetic.z = 0.0;
    if (isnan(rpm) || isinf(rpm))
        rpm = 0.0;
    if (isnan(gpsAltitude) || isinf(gpsAltitude))
        gpsAltitude = 0.0;
    ...
    if (gpsTime[0] == '\0')
        strncpy(gpsTime, "00:00:00", sizeof(gpsTime));
    if (lastCommand[0] == '\0')
        strncpy(lastCommand, "NONE", sizeof(lastCommand));
    ...
    Serial.println("====CanSat Team COSMOS====\n\nSecondary Flight Computer PID control Software as flown in CanSat Competition 2025 on 7 Jun\n(commenting changed post flight)\nFreshmen student team from the University of Alabama in Huntsville's Space Hardware Club\n\nAuthors:\n  - Caleb Wiley (logic, debugging, testing, tuning, loop optimization, camera control,\n                data-management, magnetometer callibration and tilt-compensated heading,\n                and efficiency state-switch)\n  - Barrett Twinning (camera control, sensor integration, servo integration, and libraries)\n  - Luke Wiggins (final tuning, damping, and filtering)\n\n// Relics from previous versions have not been removed\n// Custom hardware designed by Barrett Twinning");
