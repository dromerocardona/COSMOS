...
Secondary Flight Computer PID control Software as flown in CanSat Competition 2025 on 7 Jun (commenting changed post flight)
Freshmen student team from the University of Alabama in Huntsville's Space Hardware Club

Authors:
    - Caleb Wiley (logic, debugging, testing, tuning, loop optimization, camera control, data-management, magnetometer callibration and tilt-compensated heading, and efficiency state-switch)
...
#define TEAM_ID "3195"
#define PULSECOMS A4 // pin for communicating with main FC
//------------------------------------------run cam---------------
...
{
    float temp[100]; // adjust size if needed
    if (size > 100)
        return 0.0; // hard fail if overflow

    // Copy original array to preserve order
    for (int i = 0; i < size; i++)
    {
        temp[i] = arr[i];
    }

    // Simple selection sort — slow but predictable, good for small n
...
// main execution of PID loop (includes relics)
...
void SingleShotMeasure_setup()
{
    ...
    ens220.setPressureConversionTime(ENS220::PressureConversionTime::T_4_1); // sacrifice accuracy for speed

    // Use lower oversampling on pressure and temperature:
    // N_4 cuts the filter overhead in half; N_2 cuts it to one-quarter; N_1 is fastest.
    // Here we pick N_4 as a good tradeoff.
    ens220.setOversamplingOfPressure(ENS220::Oversampling::N_4);
    ens220.setOversamplingOfTemperature(ENS220::Oversampling::N_4);

    // record temp less often
...
void setup()
{
    ...
    delay(5000); // pysical safety delay: !!SERVO STARTS!!
...
void loop()
{
    ...
    if (avg(velocityHistory, historySize) < 0)
    {
        StabMode = true;
    }
    else
    ...
    // median(altitudeHistory[], len(altitudeHistory[]));

    // split modes for efficiency
    if (!StabMode)
    {
        Serial.print(millis());
        ...
        camServo.writeMicroseconds(1490);
...
// unused flush that could be implemented on mission end and power-off (no mission end implemented)
void flushBuffer()
{
